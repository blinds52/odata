...
public class TranslateAst extends RunTests{
	 ...
	 */
	public TranslateAst(String testName, PrintStream out){super(testName, out);}
	...
	class AstTranslator extends AstCallback{
		...
		void enableSkip(boolean enable){skip = enable;}
		...
		@Override public boolean preBranch(int offset, int length){
			...
			if(length == 0){out.println("<empty>");}
			else{out.println(Utilities.charArrayToXml(callbackData.inputString, offset, length));}
