...
class Utils {
	...
	static class CommandLineParser{
		...
		CommandLineParser(String cmd, Flag[] flags, Parameter[] params, boolean caseSensitive){
			if(flags == null){this.flags = new Flag[0];}
			else{this.flags = flags;}
			if(params == null){this.params = new Parameter[0];}
			else{this.params = params;}
			if(cmd == null || cmd.equals("")){this.cmd = "command";}
			else{this.cmd = cmd;}
		...
		static class Flag{
			...
			public Flag(String name, String arg, String description) throws IllegalArgumentException{
				boolean ok = true;
				if(name == null || name.equals("")){ok = false;}
				if(description == null || description.equals("")){ok = false;}
		...
		static class Parameter{
			...
			public Parameter(String name, String prefix, String description, String defaultValue) throws IllegalArgumentException{
				boolean ok = true;
				if(name == null || name.equals("")){ok = false;}
				if(prefix == null || prefix.equals("")){ok = false;}
				if(description == null || description.equals("")){ok = false;}
				if(!ok){throw new IllegalArgumentException();}
				this.name = name;
				this.arg = prefix;
		...
		boolean parse(String[] args){
			...
			try{
				...
				for(String tok : args){
					...
					if(!matched){throw new Exception(exceptionMsg(tokIndex, tok, "unrecognized argument"));}
				}
			} catch(Exception e){
