 ...
 * The statistics count the number of times each node
 * was visited (hit) by the parser in its journey through the syntax tree
 * and the result of each hit - that is, whether the node matched a phrase of the 
 * input string or not.  
 */
public class Statistics {
	 ...
	 * Instances of a Parsers will sometimes be rerun several or many times
	 * on different strings or with different parameters of some sort.
	 * Occasionally the user will wish to see the total statistics of all
	 * parses. If cumulation is enabled, the Statistics class will collect
	 * a running sum of the statistics of all parses of the same instance of the Parser.
	 * @param enable if <code>true</code>, Statistics will cumulate statistics for all parses
	 * of the Parser instance. If <code>false</code>, the statistics will be reset to zero (0)
	 * for each parse. The default value is <code>enable=false</code>.
	 */
	public void enableCumulate(boolean enable){cumulate = enable;}
	/**
	 * Returns the hit count for the requested node type.
	 * @param type determines the type of hit count to return.<br />
	 * <code>"match"</code> - hit count for nodes that were matched with non-empty phrases<br />
	 * <code>"empty"</code> - hit count for nodes that were matched with empty phrases<br />
	 * <code>"nomatch"</code> - hit count for nodes that were did not match any phrase at all<br />
	...
	public int getHits(String type) throws Exception{
		...
		while(true){
			...
			if(first == 'm' || first == 'M'){ret = totals.match;break;}
			if(first == 'n' || first == 'N'){ret = totals.nomatch;break;}
			if(first == 'e' || first == 'E'){ret = totals.empty;break;}
			if(first == 't' || first == 'T'){
	 ...
	 * <code>"operators"</code> - display node hits for all operator types except rule and UDT<br />
	 ...
	 * <code>"rules"</code> - display node hits for rule name nodes only<br />
	 * <code>"udt"</code> - display node hits for UDT nodes only<br />
	 * @param alpha if <code>true</code>, the rule or UDT nodes are displayed in alphabetical order,
	 * if <code>false</code>, rule or UDT nodes are ordered descending on hit count.
	 * @throws Exception thrown if the type is none of the above.
	 */
	...
	/**
	 * Displays a detailed table of hit counts for all operator and hit count types.
