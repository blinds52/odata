...
<p>
The functions in this class allow the user to define which nodes appear in the AST,
traversal of the AST performing user-defined callback functions for translation of the parsed sentence,
and output of the AST in native or XML format.
</p>
<p>
With the XML format the user is freed from the APG translation convention and may use
the XML parser of his/her choice for translation. 
 ...
 */
	...
	// public API
		 ...
		 * giving the user access to the input string and optionally any other user-defined
		 * data that the function may need to do its job.
		 */
	public class CallbackData{
		/**
		 * User-defined data that the user sets prior to parsing with the parser function
		 * setMyData(). Otherwise, null.
		 */
		public Object myData = null;
		/**
		 * The sentence or input string (as a char array) being parsed.
		 * Any changes made to this string during translation will render the results
		 * of the translation unpredictable.
		 */
		public char[] inputString = null;
	}
	/**
	 * The base class for all AST callback functions. It is very similar to the callback
	 * functions used for semantic actions called by the Parser.
	 */
	public static abstract class AstCallback{
		/**
		 * Provides user access to the sentence being parsed, as well as any user-defined
		 * data that may be necessary.
		 */
		public final CallbackData callbackData;
		/**
		 * 
		 * @param ast an instance of the AST this callback function is to be attached to.
		 */
		public AstCallback(Ast ast){
			this.callbackData = ast.callbackData;
		}
		/**
		 * This function is called when this AST node is reached the first time and 
		 * before the AST branch below this node is traversed.
		 * @param offset offset into the input string of the first character
		 *  of the phrase matched by this node.
		 * @param length the number of characters in the matched phrase.
		 * @return if <code>true</code> AST traversal will skip processing 
		 * of all nodes in the AST branch below this node.
		 * if <code>false</code> AST traversal continues normally.
		 */
		public boolean preBranch(int offset, int length){return false;}
		/**
		 * This function is called when this AST node is reached the second and final time and 
		 * after the AST branch below this node has been traversed.
		 * @param offset offset into the input string of the first character
		 *  of the phrase matched by this node.
		 * @param length the number of characters in the matched phrase.
		 */
	 ...
	 * The underscore insures that it will never conflict with an node name.
	 * The AST and its XML representation is always required to have a root node
	 * that is the last ancestor of every other node in the tree. Since the user
	 * may choose not to have the start rule in the AST, an assigned root node is necessary.
	 * <p>
	 * With this function the user may assign a callback function for the root node.
	 *  
	 ...
	 * @param id identifier of the rule to set the callback for.
	 * This is the id assigned to the rule in the generated Grammar class with the 
	 * ruleID() function.
	 * @param callback the user-defined callback function for this rule node.
	 * Must be an extension of the AstCallback class.
	 * @throws Exception Exceptions are thrown if the id is out of range, <code>0-(ruleCount - 1)</code>
	 * or id this rule node has not been set (@see {@link #enableRuleNode}) 
	 */
	public void setRuleCallback(int id, AstCallback callback) throws Exception{
		...
		if(!astNodes[id].enabled){throw new Exception("AST node not set for this rule: id: "
	 ...
	 * @param id identifier of the rule to set the callback for.
	 * This is the id assigned to the rule in the generated Grammar class with the udtID() function.
	 * @param callback the user-defined callback function for this rule node.
	 * Must be an extension of the AstCallback class.
	 * @throws Exception Exceptions are thrown if the id is out of range, <code>0-(udtCount - 1)</code>
	 * or id this UDT node has not been set (@see {@link #enableUdtNode}) 
	 */
	public void setUdtCallback(int id, AstCallback callback) throws Exception{
		...
		if(!astNodes[astId].enabled){throw new Exception("AST node not set for this UDT: id: "
	 ...
	 * @param enable if <code>true</code>, the rule node will be retained in the AST,
	 * if <code>false</code>, the rule node will never appear in the AST.
	 * @throws Exception Exceptions are thrown if the id is out of range, <code>0-(ruleCount - 1)</code>
	 */
	 ...
	 * @param enable if <code>true</code>, the UDT node will be retained in the AST,
	 * if <code>false</code>, the UDT node will never appear in the AST.
	 * @throws Exception Exceptions are thrown if the id is out of range, <code>0-(udtCount - 1)</code>
	 */
	 ...
	 */
	public void setMyData(Object data){callbackData.myData = data;}
	/**
	 * Traverse the AST and call the user-defined callback functions at each node.
	 * Nodes must have been identified with a call to enableRuleNode() or enableUdtNode().
	 * Additionally, callback functions must have been set for the nodes with calls to
	 * setRuleCallback() or setUdtCallback();
	...
	public void translateAst(){
		...
		for(int i = 0; i < stack.size(); i++){
			...
			if(callback != null){
				if(record.up){
					if(skip && skipDepth == record.depth){skip = false;}
					if(!skip){callback.postBranch(record.offset, record.length);}
				}
				else{
					if(!skip){
