...
import com.coasttocoastresearch.apg.Utilities;
import com.coasttocoastresearch.examples.Utils.CommandLineParser;
import com.coasttocoastresearch.examples.Utils.CommandLineParser.Flag;
import com.coasttocoastresearch.examples.Utils.CommandLineParser.Parameter;
import com.coasttocoastresearch.examples.demo.*;
import com.coasttocoastresearch.examples.anbn.RunAnBn;
import com.coasttocoastresearch.examples.anbncn.RunAnBnCn;
import com.coasttocoastresearch.examples.expressions.RunExpressions;
import com.coasttocoastresearch.examples.inifile.RunIniFile;
import com.coasttocoastresearch.examples.mailbox.RunMailbox;
import com.coasttocoastresearch.examples.testudtlib.RunUdtTest;
/** This class provides a static main() function which is the driver 
 * for selecting from among the several available tests.
 * Interested persons should look at the example code for details of how 
 * to use Java APG parsers for the examples given.
 * Some of the examples provide simple demonstrations of usage.
 * Most of the examples give a comparison between the normal CFG parsers
 * and the same problem using UDT functions for greater efficiency.
 * The comparisons are done with the standardized functions in {@link RunTests RunTests}
<table style="border:1px solid black;border-collapse:collapse;float:right;" >
<tr><th  style="border:1px solid black;">Timing Test Name</th><th  style="border:1px solid black;">factor</th></tr>
 ...
 * <p>
 * A quick comparison of the average time factors for the examples
 * is given in the table to the right. The factor is CFG time / UDT time.
 * Results will vary in different environments.
<p>
...
<i>
Disclaimer: These examples should not be considered a part of the Java APG API.
Backward compatibility or even the existence of these examples or packages 
from version to version is not guaranteed.
</i>
 */
//</table>
//<table>
...
public class Main {
	 ...
	 * Selects a test from the first command line parameter and runs the test.
	 * Catches all Exceptions thrown by the test and displays the Exception message
	 * and a stack trace of where it was thrown from. 
	 * @param args the command line parameters for test selection.
	 * Request help (arg=?) to see all command line parameters and valid test names.
	 */
	public static void main(String[] args) {
        ...
        try{
            ...
            if(data.testName.equalsIgnoreCase(Tests.ALL.getName())){runAll(data);}
            else if(data.testName.equalsIgnoreCase(Tests.DEMOS.getName())){runDemos(data);}
            else if(data.testName.equalsIgnoreCase(Tests.TIMING.getName())){runTiming(data);}
            else{
            	theTest = selectTest(data.testName, data.reps, data.out);
    	        if(theTest == null){throw new Exception("unrecognized test name \""+data.testName+"\"");}
    	        theTest.run();
	...
	private static void runAll(TestData data) throws Exception{
		...
		for(Tests test : Tests.values()){
			...
			default:
				...
				else{throw new Exception("Main: runAll: test "+test.getName()+" not found");}
				break;
			}
		}
	}
	...
	private static void getCommandlineParameters(String[] args, TestData data) throws Exception{
		...
		if(params[REPETITIONS].getValue() != null){
			try{
				...
				data.reps = r;
			} catch(Exception x){throw new Exception("repetitions: \""+
