...
import com.coasttocoastresearch.apg.UdtLib;
import com.coasttocoastresearch.apg.Utilities;
import com.coasttocoastresearch.examples.RunTests;

/** A simple demonstration of how to display the trace 
of the parser's path through the syntax tree.
The source code should be consulted for details.
 */
public class DisplayTrace extends RunTests{
	/**
	 ...
	 */
	public DisplayTrace(String testName, PrintStream out){super(testName, out);}
	...
	@Override protected void run() throws Exception{
		...
		outputAbstract("Use the Expressions grammar to " +
				"demonstrate the trace display.");
		out.println("         : Can you find where u_id fails because the identifier" +
				" is not alphanumeric?");
		out.println();
		...
		String inputString = "(a+b)*(c+(d+e*(topA+#@!^)))";
