...
abstract class Opcode {
	...
	void execute(SystemData data) throws Exception{
		...
		if(data.trace != null){data.trace.traceDown(data.inputString, offset, this);}
		if(data.ast != null){astSize = data.ast.getStackSize();}
		
		// execute the opcode-type-specific operations
		executeOpcode(data);

		// post-execution
		if(data.maxMatchedLength < data.offset){data.maxMatchedLength = data.offset;}
		if(data.trace != null){data.trace.traceUp(data.inputString, offset, data.offset - offset, data.match, this);}
		if(data.ast != null && !data.match){data.ast.setStackSize(astSize);}
		if(data.stats != null){data.stats.incrementOperator(type, data.match, data.offset - offset);}
		data.treeDepth--;
	}
	/**
	 * 
	 * @param data
	 * 
	 * executeOpcode must return:
	 * SystemData.match = true/false
	 * SystemData.offset = offset to first unmatched string character (eg. input offset + matched phrase length)
	 */
	...
	static final class Alt extends Opcode {
		...
		void executeOpcode(SystemData data) throws Exception{
			for(int i = 0, n = children.length; i < n; i++){
				...
				if(data.match){break;} // prioritized-choice - returns first successful child state and phrase
...
//					if(data.stats != null){data.stats.incrementBacktrack(type);} // NOMATCH backtracks over failed child
			...
			if(empty){data.match = true;}//REP always succeeds on empty regardless of min,max values
			else if((reps >= (int)min) && (reps <= (int)max)){data.match = true;}// number of matched reps OK
			else {
				data.match = false;
	...
	static final class Rnm extends Opcode {
		...
		void executeOpcode(SystemData data) throws Exception{
			...
			if(callback != null && returnLength < 0){
				...
				int phraseLength = -1;
				if(data.match){phraseLength = data.offset - offset;}
	...
	static final class Udt extends Opcode {
		...
		void executeOpcode(SystemData data) throws Exception{
			...
			int phraseLength = 0;
			if(data.ast != null && data.inPredicate == 0){data.ast.down(true, id, name, astOffset);}
			if(data.offset >= data.inputString.length){data.match = mayBeEmpty;}// handle EOS as empty string
			else{
				...
				if(phraseLength < 0){data.match = false;}
				else if(phraseLength == 0){data.match = mayBeEmpty;} // enforce empty string specification (e_udtName)
				else{
					data.match = true;
		...
		@Override public String toString(){return String.format("UDT(%d, %s)", id, name);}
	}
		...
		void executeOpcode(SystemData data){
			...
			while(true){
				...
				for(i = 0; i < string.length; i++){
					...
					if(cInput >=65 && cInput <= 90){cInput += 32;} // convert to lower case
					if(cInput != string[i]){
						data.match = false; // unsuccessful phrase match
						break;
					}
